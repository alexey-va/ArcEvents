...
private val TITLE_COLOR = TextColor.fromHexString("#f4bd6a")!!
private val ACTION_COLOR = TextColor.fromHexString("#d7b486")!!
...
internal class ArcEventsDialogMenu(
    ...
    private fun screen(player: Player, titleKey: String, bodies: List<PaperDialogBody>, buttons: List<PaperDialogButton>, columns: Int, id: String) =
        PaperDialogScreen(nativeText(locale.render(titleKey, player)).color(TITLE_COLOR), body = bodies.map { it.copy(text = nativeText(it.text)) }, buttons = buttons, columns = columns,
    ...
    private fun body(player: Player, nameKey: String, loreKey: String, values: Map<String, Component> = emptyMap()) =
        PaperDialogBody(Component.join(JoinConfiguration.newlines(), listOf(locale.render(nameKey, player, values)) + locale.lore(loreKey, player, values).map(TttItems::nonItalic)).color(BODY_COLOR), 468)

    private fun nativeText(value: Component): Component = value
        .color(when (value.color()?.value()) {
            0x92bed8 -> ACTION_COLOR
            0xe6fff3 -> BODY_COLOR
            0x8c8c8c, 0x969696, 0xaaaaaa -> MUTED_COLOR
            0x79c99e -> TextColor.color(0x9bd48d)
            else -> value.color()
        })
        .decoration(TextDecoration.ITALIC, false)
        .children(value.children().map(::nativeText))

    private fun button(player: Player, nameKey: String, loreKey: String, view: EventsView, slot: Int, values: Map<String, Component> = emptyMap(), width: Int = 230) =
        PaperDialogButton(PaperDialogActionId.of(if (slot < 0) "back" else "action_" + nameKey.replace(Regex("[^a-z0-9_]"), "_").take(38)), nativeText(locale.render(nameKey, player, values)).color(if (slot < 0) MUTED_COLOR else ACTION_COLOR),
            tooltip = Component.join(JoinConfiguration.newlines(), locale.lore(loreKey, player, values).map(TttItems::nonItalic)).color(MUTED_COLOR), width = width,
            onClick = PaperDialogClickHandler { context -> if (slot >= 0 && context.player.isOnline) dispatch(context.player, view, slot) })
}
